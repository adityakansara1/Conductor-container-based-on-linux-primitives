...
run()
{
    ...
    # This is required for the container to have access to devices within the host
    ...
    unshare --fork -p -u -n -m -i --kill-child chroot $CONTAINERDIR/$NAME/rootfs /bin/bash -c "chmod 755 / && mount -t sysfs sysfs /sys && mount -t proc proc /proc && $INIT_CMD_ARGS"
}
...
exec()
{
    ...
    # nsenter --root=$CONTAINERDIR/$NAME/rootfs/ --wd=/proc/$CONTAINER_INIT_PID/cwd --target $CONTAINER_INIT_PID -m -u -i -p -n $EXEC_CMD_ARGS #--mount=/proc/$CONTAINER_INIT_PID/ns/mnt --uts=/proc/$CONTAINER_INIT_PID/ns/uts --ipc=/proc/$CONTAINER_INIT_PID/ns/ipc --net=/proc/$CONTAINER_INIT_PID/ns/net --pid=/proc/$CONTAINER_INIT_PID/ns/pid $EXEC_CMD_ARGS
    nsenter -m -u -i -p -n -r -w --target $CONTAINER_INIT_PID -- $EXEC_CMD_ARGS
...
addnetwork()
{
    ...
    ip link add name "$INSIDE_PEER" type veth peer name "$OUTSIDE_PEER"
